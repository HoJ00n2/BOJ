...
# 재귀하는 파라미터로 좌표 (x,y)와 영역 길이 정보 n 을 준다.
# 재귀할 때 마다 영역 내 정보 같은 정보인지 check
# 각 영역을 탐색해야 하므로 4번의 재귀로 표현한다. (왼위, 오위, 왼아래, 오아래) >> 탐색순서 중요

def check(x, y, n):
    one = 0 # 영역 내 1의 개수
    zero = 0 # 영역 내 0의 개수
    # 해당 영역 내의 값이 모두 같은지 아닌지는 모두 여기서 판단
...
check(0,0,n) # 처음은 쪼개지 않고 시작 (모두 0이거나 1일 수 있으므로)
